...
void tarjan(int m) {
    ...
    vindex.resize(2*m + 1, -1);
    vlowlink.resize(2*m + 1, -1);
    vonstack.resize(2*m + 1, false);
    stack<int> s;
    int index = 0;
    repae(i, 1, 2*m) {
        if (vindex[i] == -1) {
...
void solve() {
    ...
    vector<tuple<char, int, char, int>>choice;
    rep(i, n) {
        ...
        //aff nai espaff bi endf
        //aff nbi espaff ai endf
    ...
    /*repae(i, 1, 2 * m) {
        if (i <= m) {
            aff "i" espaff i espaff comp[i] endf
        }
        else {
            aff "ni" espaff i-m espaff comp[i] endf
        }
    }*/
